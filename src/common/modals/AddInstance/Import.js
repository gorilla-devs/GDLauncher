...
const Import = ({
  ...
  const onClick = async () => {
    ...
    if (isForge) modloader.loaderType = FORGE;
    else if (isFabric) modloader.loaderType = FABRIC;
    else if (isVanilla) modloader.loaderType = VANILLA;

    if (manifest.projectID) {
      modloader.addonId = manifest.projectID;
    }
