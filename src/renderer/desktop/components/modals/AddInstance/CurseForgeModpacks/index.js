...
import { getSearch } from 'src/common/api';

let lastRequest;
const CurseForgeModpacks = ({ setStep }) => {
  const mcVersions = useSelector(state => state.vanillaManifest?.versions);
  const categories = useSelector(state => state.curseforgeCategories);
